...
for region in reg_lst:
    ...
    oce_df = oce_df/1000
    Reg_mb_df = Reg_mb_df/1000
    Reg_sig_mb_df = Reg_sig_mb_df/1000
    ...
    if end_plot == '_OCE_and_B_reg_':
        ...
        plt.fill_between(Reg_mb_df.index, Reg_mb_df[region] + Reg_sig_mb_df[region],
                         Reg_mb_df[region] - Reg_sig_mb_df[region], color='grey', alpha=0.5, linewidth=0)

    # plot regional B

    ax.plot(Reg_mb_df[region], color='black', label='Aw mean i_obs', alpha=0.7, linewidth= 1)
    ...
    ax.axhline(0, color='Grey', linewidth=1)
    ax.set_xlabel('Year')
    ax.set_ylabel(r'B$_{R-OCE}$ (m w.e. yr$^{-1}$)', fontsize='large')
    ax.text(1990, 3.8, 'N$_{gla}$ = ' + str(nb_gla_reg) + ' glaciers')
    ax.text(1990, 3.2, 'N$_{obs}$ = ' + str(nb_gla_obs) + ' glaciers, '+ str("{:.2f}".format((nb_gla_obs*100)/nb_gla_reg))+' %')
...
Reg_mb_df.to_csv(out_dir + 'Regional_B_series_mwe_AreaWeighted.csv')
Reg_sig_mb_df.to_csv(out_dir + 'Regional_B_series_mwe_uncertainty.csv')
